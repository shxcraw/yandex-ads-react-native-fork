package com.yandexmobileads.app_open
...
internal class YandexAppOpenAdModule(
    private val context: ReactApplicationContext,
    private val eventEmitter: EventEmitter = EventEmitter(context, NativeMapFactory()),
    private val loadersStorage: ObjectStorage<AppOpenAdLoader> = ObjectStorage(),
    private val adsStorage: ObjectStorage<AppOpenAd> = ObjectStorage(),
    ...
    fun newLoader(promise: Promise) {
        reactApplicationContext.currentActivity?.runOnUiThread {
            val loader = AppOpenAdLoader(reactApplicationContext)
            val loaderId = loadersStorage.put(loader)
            loader.setAdLoadListener(RNAppOpenAdLoadListener(loaderId))
    ...
    inner class RNAppOpenAdLoadListener(
        ...
        override fun onAdLoaded(appOpenAd: AppOpenAd) {
            val id = adsStorage.put(appOpenAd)
            appOpenAd.setAdEventListener(RNAppOpenAdEventListener(id))
            loadPromises[loaderId]?.poll()?.resolve(id)
        }
    }

    inner class RNAppOpenAdEventListener(
    ...
    companion object {
        const val NAME = "YandexAppOpenAdModule"
    }
}
